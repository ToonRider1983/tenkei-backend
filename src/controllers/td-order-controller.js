...
exports.Search_Order_No_AfterUpdate = async (req, res, next) => {
    try {
        ...
        if (typeof orderNo !== 'string' || orderNo.length < 10) {
            return next(createError(400, "Order number is required and must be at least 10 characters long"));
        }
...
exports.tm_workerg = async (req, res, next) => {
    try {
        ...
        const workerg = await prisma.tM_WorkerG.findMany({
            select: {
                WorkG_CD: true,  // เปลี่ยนคอลัมน์ที่ต้องการดึงข้อมูลที่นี่
            },        });
...
exports.tm_customer= async (req, res, next) => {
    try {
        ...
        const customer = await prisma.tM_Customer.findMany({
            select: {
                Customer_CD: true,  // เปลี่ยนคอลัมน์ที่ต้องการดึงข้อมูลที่นี่
            },      
          });
    ...
    try {
        ...
        const updatedOrder = await prisma.tD_Order.update({
            ...
            data: {
                ...
                NAV_Size: orderData.Product_Size // อัปเดต NAV_Size
            },
        });
...
exports.Product_Docu_Set = async (req, res, next) => {
    try {
        ...
        if (!Order_No || !Production_Docu_Drive) {
            return res.status(400).json({ message: "Order_No and Production_Docu_Drive are required" });
        }
        ...
        // สร้างเส้นทางเอกสาร
        const productDocPath = `#${Production_Docu_Drive}\\${year}\\${Order_No.slice(5, 7)}\\${Order_No.slice(0, 3)}\\${Order_No}${productDocType}#`;
        ...
        // ส่งคำตอบกลับ
        return res.status(200).json({ message: "Product document updated successfully", order: updatedProduct });
    } catch (err) {
        // ล็อกข้อผิดพลาดเพื่อการตรวจสอบ
